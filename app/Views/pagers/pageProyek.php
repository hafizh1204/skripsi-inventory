...
<nav aria-label="Page navigation">
    <ul class="pagination flex items-center -space-x-px h-8 text-sm">
        <?php if ($pager->hasPrevious()) : ?>
            <li>
                <a href="<?= $pager->getFirst() ?>" aria-label="<?= lang('Pager.first') ?>" class="flex items-center justify-center px-3 h-8 ms-0 leading-tight text-gray-500 bg-white border border-e-0 border-gray-300 rounded-s-lg hover:bg-gray-100">
        ...
        <?php foreach ($pager->links() as $link): ?>
            <li class="z-10 flex items-center justify-center px-3 h-8 leading-tight text-slate-600 border border-slate-300 bg-slate-50 hover:bg-slate-100 hover:text-slate-700 <?= $link['active'] ? 'text-green-400 bg-primary-green font-semibold text-lg' : ''  ?> ">
                <a href="<?= $link['uri'] ?>">
                    <?= $link['title'] ?>
                </a>
            </li>
